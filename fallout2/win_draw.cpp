...
static int handle(MSG& msg) {
	...
	case WM_MOUSEMOVE:
		...
		if(canvas && size.x)
			hot.mouse.x = (short)((long)(hot.mouse.x) * (long)canvas->width / (long)size.x);
...
void draw::create(int x, int y, int width, int height, unsigned flags, int bpp) {
	...
	if(x == -1)
		x = (screen_w - (MinimumRect.right - MinimumRect.left)) / 2;
	if(y == -1)
		y = (screen_h - (MinimumRect.bottom - MinimumRect.top)) / 2;
