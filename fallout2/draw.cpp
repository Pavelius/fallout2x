...
// Metrics
sprite*				metrics::font = (sprite*)loadb("art/font1.pma");
sprite*				metrics::h1 = (sprite*)loadb("art/font2.pma");
sprite*				metrics::h2 = (sprite*)loadb("art/font3.pma");
		...
		while(sb < se) {
			if(sb->a == 0xFF) {
				...
				*d2++ = *sb++;
				...
				auto ap = sb->a;
				d2->r = (((int)d2->r * ap) + ((sb->r) * (255 - ap))) >> 8;
				d2->g = (((int)d2->g * ap) + ((sb->g) * (255 - ap))) >> 8;
				d2->b = (((int)d2->b * ap) + ((sb->b) * (255 - ap))) >> 8;
				d2++; sb++;
			}
		}
		s += s_scan;
...
bool draw::buttonp(const char* title, unsigned key, fnbutton proc, bool vertical) {
